...
# If you modify this file, please pass it through:
#
#   flake8 --ignore=E501,W503 generate-comment-diff.py
#   black generate-comment-diff.py
#
# And test with:
#
#   ./generate-comment-diff-test.sh
...
def print_range_header(diff_slice):
    # Print a diff hunk-like header that indicates where the following lines
    # come from in a and b versions of the file.
...
def print_diff_with_comments(server, diff, comments, revision):
    ...
    # Compute a list of ranges of `diff_lines` we want to print, based on
    # where the comments are.
    #
    # FIXME: This is broken for overlapping range comments, we should use a
    # proper rangeset to merge ranges.
    for comment in comments:
        if comment.line is None:
        ...
        # We want to print from this point.
...
def main():
    ...
    # It is possible to comment on multiple revisions of a change at the same
    # time...  so generate different diffs (each relative to the base) for each
    # revision that was commented on.
    for (revision, comments_by_path) in comments_by_revision.items():
        for (path, comment_for_path) in comments_by_path.items():
            # Get the information required to build the diff
            diff_from_base_to_rev = server.get_diff(change, revision, path)

            # Print it with interleaved comments.
            print_diff_with_comments(
